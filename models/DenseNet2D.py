...
class DenseNet(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # print(type(x)) # (tensor)
        # print(x.shape) # torch.Size([2, 1, 64, 64, 64])

        features = self.features(x) # torch.Size([2, 1024, 2, 2, 2])
        out = F.relu(features, inplace=True)
        # out = F.adaptive_avg_pool3d(out, (1, 1, 1)) # torch.Size([2, 1024, 1, 1, 1])
        out = self.avgpool(out) # torch.Size([2, 1024, 1, 1, 1])
...
def densenet161(*, weights = None, progress: bool = True, **kwargs: Any) -> DenseNet:
...
def densenet169(*, weights = None, progress: bool = True, **kwargs: Any) -> DenseNet:
...
def densenet201(*, weights = None, progress: bool = True, **kwargs: Any) -> DenseNet:
...
class DenseNetWrapper(nn.Module):
    def __init__(self, backbone, output_dims=2, model_name = "densenet121", in_channels=3, fc_in=None, joint_fusion=False):
    ...
    def replace_fc(self, module, name, new_module):
        for attr_str in dir(module):
            ...
            if type(target_attr) == torch.nn.Linear:
                print('replaced:', name, attr_str, new_module)
